...
type GfToken struct {
	...
	//忽略拦截地址
	AuthIgnorePaths g.SliceStr
	// 认证验证方法 return true 继续执行，否则结束执行
	AuthBeforeFunc func(r *ghttp.Request) bool
	// 认证返回方法
	AuthAfterFunc func(r *ghttp.Request, respData Resp)
	//跨域中间件
	CorsMiddleware func(r *ghttp.Request)

	//刷新token后执行, 目前固定每天首次认证token刷新
	RefreshTokenAfterFunc func(respData Resp)
}
...
func (m *GfToken) Init() bool {
	...
	if m.CorsMiddleware == nil {
		m.CorsMiddleware = func(r *ghttp.Request) {
			corsOptions := r.Response.DefaultCORSOptions()
			corsOptions.AllowHeaders = "*"
			corsOptions.AllowOrigin = "*"
			corsOptions.AllowDomain = nil
	...
	if m.AuthBeforeFunc == nil {
		m.AuthBeforeFunc = func(r *ghttp.Request) bool {
			...
			for _, authIgnorePath := range m.AuthIgnorePaths {
				if authIgnorePath == r.URL.Path {
					glog.Debug("忽略token拦截链接url=", r.URL.String())
					return false
				}
			}
			return true
...
func (m *GfToken) Start() bool {
	...
	if m.AuthIgnorePaths == nil {
		m.AuthIgnorePaths = g.SliceStr{}
	}
...
func (m *GfToken) GenToken(userKey string, data interface{}) Resp {
	...
	userCache := g.Map{
		...
		//"refreshTime": gtime.TimestampMilli() + m.MaxRefresh,
		"refreshTime": gtime.TimestampMilli(),
	}
	...
	if m.RefreshTokenAfterFunc != nil {
		m.RefreshTokenAfterFunc(cacheResp)
	}
...
func (m *GfToken) validToken(token string) Resp {
	...
	//nowTime := gtime.TimestampMilli()
	refreshTime := userCache["refreshTime"]

	//token每天刷新一次
	todayDateStr := gtime.Date()
	// 需要进行缓存超时时间刷新
	//if gconv.Int64(refreshTime) == 0 || nowTime > gconv.Int64(refreshTime) {
	// 每天登陆首次验证token刷新token
	if gconv.Int64(refreshTime) == 0 || todayDateStr != gtime.NewFromTimeStamp(gconv.Int64(refreshTime)).Format("Y-m-d") {
		userCache["createTime"] = gtime.TimestampMilli()
		//userCache["refreshTime"] = gtime.TimestampMilli() + gconv.Int64(m.MaxRefresh)
		userCache["refreshTime"] = gtime.TimestampMilli()
		glog.Debug("[GToken]refreshToken:" + gconv.String(userCache))
		resp := m.setCache(cacheKey, userCache)
		if m.RefreshTokenAfterFunc != nil {
			m.RefreshTokenAfterFunc(resp)
		}
		return resp
	}
