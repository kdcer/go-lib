...
func (r *Redigo) GetMode() mode.IMode {
	return r.mode
}
...
func (r *Redigo) IsConnectSuccess() (bool, error) {
	_, err := r.String(func(c redis.Conn) (res interface{}, err error) {
		v, err := c.Do("ECHO", "hello redis")
		return v, err
	})
	if err != nil {
		return false, err
